...
function scripts(text: string) {
  ...
  for (const ch of text) {
    ...
    else if (c === 0xfffd || (c >= 0xe000 && c <= 0xf8ff) || (c < 0x20 && c !== 0x09 && c !== 0x0a && c !== 0x0d)) {
      bad++;
    }
  }
...
export function detectEncoding(samples: readonly Uint8Array[]): O2Encoding | null {
  ...
  for (const encoding of ['euc-kr', 'shift-jis', 'gbk'] as const) {
    ...
    let score = 0;
    if (encoding === 'euc-kr') {
      score = s.hangul * 3 + s.cjk + s.symbol;
    }
    else if (encoding === 'shift-jis') {
      score = s.kana * 3 + s.cjk + s.symbol + s.halfKana * 0.25;
    }
    else {
      score = s.cjk * 1.5 + s.symbol * 0.75;
    }
  ...
  if (plain && latinScore(plain.decode(joined)) >= best.score) {
    return null;
  }
