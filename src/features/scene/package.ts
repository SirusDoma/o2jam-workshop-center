...
function encodeSprite(frames: EditFrame[], position: { x: number; y: number } | undefined, name: string): Uint8Array | null {
  ...
  if (position) {
    ...
    if (dx || dy) output = output.map((frame) => ({ ...frame, x: frame.x + dx, y: frame.y + dy }));
  }
  return writeSprite(
...
export function buildScenePackage(options: SceneArchiveOptions): Uint8Array {
  const editBlock = (state: ControlState): ControlState => {
    const applySet = (control: ControlEntry): ControlEntry => {
      if (control.setId === null) return control;
      if (options.dissolvedSets.has(`${state.name}:${control.setId}`)) return { ...control, setId: null };
  ...
  for (const block of [...options.list.states, ...options.list.dialogs, ...options.addedBlocks].filter(keepBlock)) {
    ...
    if (!structureChanged && !boundsChanged && hasEntry(boundFile)) continue;
  ...
  for (const lower of touchedSprites) {
    if (options.newSprites[lower]) continue;
    ...
    if (bytes) sprites.set(lower, bytes);
  }
  ...
  const entries = archive.entries
    ...
    .map((entry) => {
      ...
      if (controlListEntry && lower === controlListEntry.name.toLowerCase()) return { name: entry.name, data: controlListBytes };
      const bounds = boundsByName.get(lower);
      if (bounds) return { name: entry.name, data: bounds.data };
      const sprite = sprites.get(lower);
      if (sprite) return { name: entry.name, data: sprite };
  ...
  for (const [lower, bounds] of boundsByName) if (!existing.has(lower)) entries.push({ name: bounds.name, data: bounds.data });
