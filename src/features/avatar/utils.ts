...
export function buildLayers(
  ...
  if (showBase && !isCostume) {
    for (const id of baseBodyIds(gender)) {
      ...
      if (base && !wearing.some((w) => w.index === base.index)) sources.push({ item: base, overrides: {} });
    }
  }
  ...
  for (const src of sources) {
    ...
    for (const region of regions) {
      ...
      if (l) out.push({ ...l, order: layerOrder(equip, region, instrument) });
    }
  }
...
function regionLayer(
  ...
  let name = '';
  if (idx !== null && overrides[idx] !== undefined) name = overrides[idx] ?? '';
  else if (idx !== null && item.sprites[idx]?.present) name = item.sprites[idx]!.filename;
  if (!name && noneIdx !== null) {
    if (overrides[noneIdx] !== undefined) name = overrides[noneIdx] ?? '';
    else if (item.sprites[noneIdx]?.present) name = item.sprites[noneIdx]!.filename;
  }
  if (!name) return null;
...
function equipTypeOf(item: ItemEntry, instrument: SpriteInstrument): string {
  if (item.itemType === 28) return 'Costume';
  if (item.itemPart === 0) {
  ...
  if (item.itemPart in partMap) return partMap[item.itemPart]!;
