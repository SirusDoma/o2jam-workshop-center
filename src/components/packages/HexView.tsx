...
export function HexView({
  ...
  useEffect(() => {
    ...
    if (!el || !native) return;
    if (!sel) {
      if (native.anchorNode && el.contains(native.anchorNode)) native.removeAllRanges();
      return;
    }
  ...
  const enter = (index: number) => {
    if (dragging.current) setSel((s) => (s ? { ...s, b: index } : s));
  };
    ...
    <>
      ...
      <span className="hx-bytes">
        {bytes.map((b, i) => {
          ...
          if (lit(index)) return <mark key={i}>{text}</mark>;
          return (
            <span
              key={i}
      ...
      <span className="hx-ascii">
        {bytes.map((b, i) => {
          ...
          if (lit(index)) return <mark key={i}>{ch}</mark>;
          return (
            <span
              key={i}
              className={inSel(index) ? 'hx-sel' : undefined}
