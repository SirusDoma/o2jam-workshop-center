...
export function MusicSectionField({ sectionKey, field, value, onChange }: {
  ...
  if (CHECK_FIELDS.has(path)) return <CheckField value={value} onChange={onChange} />;
  const options = ENUM_FIELDS[path];
  if (options) return <EnumField value={value} options={options} onChange={onChange} />;
  if (field.type === 'char') {
  ...
  if (field.type === 'bytes') return <input className="secinput" value={String(value ?? '')} placeholder="hex" onChange={(event) => onChange(event.target.value)} />;
